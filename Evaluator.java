...
public abstract class Evaluator {
	...
	protected double onFunction(String function, Parser.Node arguments) throws Error {
		...
		if (Lexer.Token.Coma.right2left) {
			int i = 0;
			while (arguments.token == Lexer.Token.Coma) {
				args[i] = evaluate(arguments.left);
				arguments = arguments.right;
				i += 1;
			}
			args[i] = evaluate(arguments);
		} else {
			int i = args.length;
			while (arguments.token == Lexer.Token.Coma) {
				args[i -= 1] = evaluate(arguments.right);
				arguments = arguments.left;
			}
			args[0] = evaluate(arguments);
		}
